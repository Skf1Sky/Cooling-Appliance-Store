...
router.put("/admin/orders/:id/status", async (req, res) => {
  ...
  res.json({ ...updated, total: Number(updated.total), createdAt: updated.createdAt.toISOString() });
});
...
router.post("/admin/products", async (req, res) => {
  ...
  const [product] = await db
    ...
    .returning();
  res.status(201).json({ ...product, price: Number(product.price), originalPrice: product.originalPrice ? Number(product.originalPrice) : undefined });
});
...
router.put("/admin/products/:id", async (req, res) => {
  ...
  res.json({ ...product, price: Number(product.price), originalPrice: product.originalPrice ? Number(product.originalPrice) : undefined });
});
...
router.get("/admin/warranties", async (_req, res) => {
  ...
  res.json(warranties.map(w => ({ ...w, createdAt: w.createdAt.toISOString() })));
});
...
router.post("/admin/warranties", async (req, res) => {
  ...
  res.status(201).json({ ...warranty, createdAt: warranty.createdAt.toISOString() });
});
...
router.put("/admin/warranties/:id", async (req, res) => {
  ...
  res.json({ ...warranty, createdAt: warranty.createdAt.toISOString() });
});
